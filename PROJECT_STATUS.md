...
10. **Flux "Je bloque" avec indices sur demande et chrono en arrière-plan** (`AnswerControls` dans App.jsx,
    juillet 2026) : le chrono initial (réglable 0-10 min, défaut 3, réglage `hint_lock_minutes` dans la table
    Supabase `user_settings`) démarre **dès que l'exercice apparaît** (`shownAt`, capturé au montage / à chaque
    changement d'`exercise.id`), pas au moment du clic sur "Je bloque" — le temps déjà passé à chercher avant de
    cliquer compte donc dans le délai. Tant que ce délai n'est pas écoulé, cliquer sur "Je bloque" affiche juste
    un compte à rebours, aucun bouton d'indice. Une fois écoulé, un bouton "Voir l'indice N" apparaît —
    **l'indice ne s'affiche jamais tout seul, il faut cliquer pour le révéler**. Après ce clic, un nouveau chrono
    d'une minute (celui-ci déclenché par le clic, pas par l'apparition de l'exercice) se lance avant que le
    ...
    sur cet exercice). "J'ai réussi" reste cliquable à tout moment du parcours. **L'ancien affichage libre de
    tous les indices d'un coup dans `ExerciseBody` a été retiré** — les indices ne sortent plus que via ce flux.
