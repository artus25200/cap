...
correction: |
  ...
  3. Posons $a_n = (\sqrt n)^{(-1)^n}\ln\left(1+\frac{1}{\sqrt n}\right)$. On minore $a_n \geqslant
  \frac{1}{\sqrt n}\ln(1+\frac1{\sqrt n}) =: b_n \underset{+\infty}{\sim} \frac1n$, et $\sum b_n$ diverge, donc
  $\sum a_n$ diverge (par minoration, séries positives). De plus $a_n \leqslant \sqrt n \ln(1+\frac1{\sqrt n})
  \leqslant 1$ (car $\ln(1+x)\leqslant x$), donc $(a_n)$ est bornée. D'après la question 2, $R=1$.
method:
