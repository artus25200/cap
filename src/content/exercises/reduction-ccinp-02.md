...
classic: true
correction: |
  1. $f^2-f-2\mathrm{Id}=0 \iff f\circ(f-\mathrm{Id})=(f-\mathrm{Id})\circ f=2\mathrm{Id} \iff f\circ(\frac12f- \frac12\mathrm{Id}) = \mathrm{Id}$. Donc $f$ inversible, $f^{-1}=\frac12f-\frac12\mathrm{Id}$.
  ...
  3. $\mathrm{Im}(f+\mathrm{Id})\subset\ker(f-2\mathrm{Id})$ : pour $y=f(x)+x$, $(f-2\mathrm{Id})(y) = f^2(x)-f(x)-2x=0$. Par 2., $n=\dim\ker(f+\mathrm{Id})+\dim\ker(f-2\mathrm{Id})$, et par le théorème du rang,
...
method:
  ...
  content: >
    Si $P(f)=0$ avec $P=P_1P_2$, $P_1\wedge P_2=1$, le lemme des noyaux donne directement $\ker P(f) = \ker P_1(f)\oplus\ker P_2(f)$ — ici $\ker P(f)=E$ car $P$ est annulateur, donnant une décomposition de $E$ tout
